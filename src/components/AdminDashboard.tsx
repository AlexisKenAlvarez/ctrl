      ...
      <div className="flex h-auto w-full">
        {nav.map((items) => (
          <Link
            ...
            key={items.label}
            className={cn("w-fit p-4 transition-all ease-in-out duration-300  relative group", {
          ...
          >
            ...
            <div className={cn("absolute bottom-0 w-0 group-hover:w-full h-1 transition-all ease-in-out duration-300 left-0 right-0 mx-auto", {
                "bg-orange-500": items.label === "pending",
                "bg-green-500": items.label === "accepted",
                "bg-red-500": items.label === "rejected",
                "w-full": searchParams.get("status") === items.label,
            })}></div>
          </Link>
        ))}
      </div>
